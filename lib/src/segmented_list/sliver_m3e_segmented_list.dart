...
/// A Material 3 interactive segmented list for use in [CustomScrollView]s with selection support.
///
/// `SliverM3ESegmentedList` renders a vertical list of items, where the first and last
...
class SliverM3ESegmentedList extends StatelessWidget {
  ...
  /// The outer margin applied around the entire sliver list.
  ///
  ...
  /// The selection mode for this sliver list (none, single, or multiple).
  ...
  /// Creates a [SliverM3ESegmentedList].
  const SliverM3ESegmentedList({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    if (itemCount == 0) {
      ...
      return SliverToBoxAdapter(
        child: effectiveMargin != null
            ? Padding(padding: effectiveMargin, child: empty)
            : empty,
      );
    }
    ...
    return effectiveMargin != null
        ? SliverPadding(padding: effectiveMargin, sliver: sliver)
        : sliver;
  }
}
